...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item We define the \AASU{} as a configuration-bound, versioned unit of security evaluation: $\AASUm = (P,M,R,T,K,H,S)$.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  ...
  \item $H$ is the history/memory configuration (state backend, retention, and isolation controls),
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[comp,below=0.2cm of k] (h) {$H$ --- History / memory configuration};
    \node[comp,below=0.2cm of h] (s) {$S$ --- Skill configuration};
    \draw[->] (aasu) -- (p);
    \draw[->] (aasu) -- (m);
    \draw[->] (aasu) -- (r);
    \draw[->] (aasu) -- (t);
    ...
    \draw[->] (aasu) -- (s);
  \end{tikzpicture}
  \caption{The Atomic AI Security Unit (\AASU{}) as a configuration snapshot. Any change to $P,M,R,T,K,H,S$ yields a distinct \AASU{} that requires separate assurance.}
  \label{fig:aasu}
\end{figure}
...
\begin{equation}
  \textsf{AASU\_ID} = H(\textsf{CanonicalSerialize}(P,M,R,T,K,H,S))
\end{equation}
...
We recommend threat models that explicitly enumerate assets, actors, trust boundaries, and data flows across $P,M,R,T,K,H,S$ and across edges.
...
In \AASU{} terms, many technique families can be localized to one or more of $P,M,R,T,K,H,S$ and to graph edges (e.g., persistence via memory, lateral movement via cross-agent pivots),
...
deploy the certified snapshot, monitor and retain evidence, and trigger recertification on any change to $P,M,R,T,K,H,S$ or key dependencies.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[step] (dev) {Define / update\\$P,M,R,T,K,H,S$};
