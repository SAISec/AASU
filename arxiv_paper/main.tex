...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item We define the \AASU{} as a configuration-bound, versioned unit of security evaluation: $\AASUm = (P,M,R,T,K)$.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \draw[->] (aasu) -- (k);
  \end{tikzpicture}
  \caption{The Atomic AI Security Unit (\AASU{}) as a configuration snapshot. Any change to $P,M,R,T,$ or $K$ yields a distinct \AASU{} that requires separate assurance.}
  \label{fig:aasu}
\end{figure}
...
\begin{equation}
  \textsf{AASU\_ID} = H(\textsf{CanonicalSerialize}(P,M,R,T,K))
\end{equation}
...
We recommend threat models that explicitly enumerate assets, actors, trust boundaries, and data flows across $P,M,R,T,K$ and across edges.
...
In \AASU{} terms, many technique families can be localized to one or more of $P,M,R,T,K$ and to graph edges (e.g., persistence via memory, lateral movement via cross-agent pivots),
...
deploy the certified snapshot, monitor and retain evidence, and trigger recertification on any change to $P,M,R,T,K$ or key dependencies.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[step] (dev) {Define / update\\$P,M,R,T,K$};
